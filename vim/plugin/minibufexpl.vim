...
"  Last Change: Friday, October 21, 2011
"      Version: 6.4.3
    ...
    let l:winFound = 1
      ...
      if l:bufNum != -1
        if g:miniBufExplVSplit == 0
          let l:split = "sb " . l:bufNum
        else
          let l:split = "vert sb " . l:bufNum
        endif
      else
        if g:miniBufExplVSplit == 0
          let l:split = "sp " . a:bufName
        else
          let l:split = "vsp " . a:bufName
        endif
      endif
...
function! <SID>IgnoreBuffer(buf)
  ...
  if (getbufvar(a:buf, '&modifiable') != 1 || l:BufName == '-MiniBufExplorer-' || l:BufName == 'MiniBufExplorer.DBG')
    return 1
  endif
